...
async function main() {
  ...
  try {
    ...
    if (!seen.includes('tool_call:bazi')) { console.error('[smoke] 失败：模型未调用 bazi'); process.exitCode = 1; return }
    if (!/庚午/.test(r.finalText || text)) { console.error('[smoke] 失败：回复未引用四柱'); process.exitCode = 1; return }
