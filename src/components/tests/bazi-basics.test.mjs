...
test('中文八字入门页展示术语、FAQ 与真实排盘入口', async () => {
  ...
  let opened = 0
  const r = render(BaziBasicsPage, { onBack: () => {}, onStartCalculator: () => { opened += 1 } })
  try {
    assert.ok(r.text().includes('八字是什么？'))
    assert.ok(r.text().includes('四柱是命理测算的基础'))
    ...
    assert.match(r.text(), /免费排盘/)
    assert.match(r.text(), /带着命盘咨询元氣 AI/)
  } finally {
    r.unmount()
  }
})
